// Validate the dense 1/4/7 NUMERIC backend kernel for the QCD three-gluon vertex (quark triangle,
// full quark-gluon vertex {1,4,7} tensor basis) against the copied FormTracer oracle. As for the
// other vacuum flows, pointwise can differ by an odd loop-routing term that integrates to zero —
// the physical check is the (cos1,cos2)-integrated value.
#include "ZA3_147_kernel.hh"     // FormTracer oracle, FORM trace of the 1/4/7 vertex (refshim/)
#include "ZA3_147_num_kernel.hh" // numeric backend, the struct-7-fix kernel (gen/)
#include "shim.hpp"
...
int main() {
  ...
  auto cForm = [&](double l1, double c1, double c2, double p, double k) {
    return Form::kernel(l1, c1, c2, p, k, Nf, d.ZA3, d.ZAcbc, d.ZA4, d.ZAqbq1, d.ZAqbq4, d.ZAqbq7,
                        d.dtZc, d.Zc, d.dtZA, d.ZA, d.dtZq, d.Zq, d.Mq);
  };
  auto cNum = [&](double l1, double c1, double c2, double p, double k) {
    return std::real(Num::kernel(l1, c1, c2, p, k, d.ZA3, d.ZAcbc, d.ZA4, d.ZAqbq1, d.ZAqbq4, d.ZAqbq7,
                                 d.dtZc, d.Zc, d.dtZA, d.ZA, d.dtZq, d.Zq, d.Mq));
  };
  ...
  std::printf("ZA3 1/4/7 numeric vs FORM:  pointwise=%.3e  (cos1,cos2)-integrated=%.3e (physical check)\n", pw, ie);
  bool ok = ie < 1e-8;
