// NumTracer — ZA3 flow integrated on GPU via KOKKOS (CUDA backend): the exact workload of
// integrate_za3_gpu.cu (same generated __host__ __device__ kernel, same GSL quadrature,
// same two-phase map/reduce, same orders 32x8x8) expressed with Kokkos parallel_for +
// team reductions instead of raw CUDA + cub — the comparison target for framework overhead.
...
struct EvalZA3 {
  DressingSet d;
  double k;
  KOKKOS_INLINE_FUNCTION double operator()(double l1, double c1, double c2, double p) const {
    return numtracer::kokkos::real_of(K::kernel(l1, c1, c2, p, k, d.ZA3, d.ZAcbc, d.ZA4, d.ZAqbq1,
...
int main(int argc, char **argv) {
  ...
  const double pref = 2.0 * M_PI / std::pow(2.0 * M_PI, 4);
  const EvalZA3 ev{DressingSet{}, k};
  DressingSet d;
  ...
  numtracer::kokkos::SegmentedQuadIntegrator<EvalZA3, NV> I;
  for (int a = 0; a < NV; ++a) I.set_axis(a, ax[a].nodes, ax[a].weights);
  auto run = [&]() {
    auto r = I.map(P, pref, ev);
    // the loop-independent constant() once per grid point (host side, as in the CUDA twin)
    for (size_t i = 0; i < P.size(); ++i)
  ...
  // --- timing at the same production orders ---------------------------------------------------
  {
    ...
    std::printf("production  orders 32x8x8, %zu p-points (%lld evals, buffer %.1f MB) [Kokkos]:\n",
