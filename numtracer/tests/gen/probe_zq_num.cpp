...
static inline auto probe_full(const double& l1, const double& cos1, const double& p, const double& k)
{
  ...
  return fma(-0.0833333333333333, _cse2 * _den3 * _den4 * DiFfRG::zq_num::tr0(fenv) * powr<2>(_interp1) * (-_cse1 * _interp13 * (_interp14 + 50. * (_interp15 - _interp4)) - _cse1 * _interp12 * _interp4), fma(complex<double>(0.,0.0833333333333333), _cse2 * _den2 * _den5 * DiFfRG::zq_num::tr1(fenv) * powr<2>(_interp1) * (_interp7 * _interp9 + _interp8 * (_interp10 + 50. * _den1 * (_interp11 - _interp7) * powr<6>(k))), 0.));
}
...
static inline auto probe_repart(const double& l1, const double& cos1, const double& p, const double& k)
{
  ...
  const double dr_8 = ntStub(85453., 1. * sqrt(powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
  DiFfRG::zq_num::fill(fenv, l1, cos1, p, dr_0, dr_1, dr_2, dr_3, dr_4, dr_5, dr_6, dr_7, dr_8);
  const auto _interp1 = ntIm(DiFfRG::zq_num::tr1(fenv));
  const auto _interp2 = ntStub(12206., 0.816496580927726 * sqrt(powr<2>(l1) - cos1 * l1 * p + powr<2>(p)));
  ...
  const auto _interp13 = ntRe(DiFfRG::zq_num::tr0(fenv));
