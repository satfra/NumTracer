...
#include "Kaon_tadpole_tflav_kernels.hh"

namespace DiFfRG
{
  class Kaon_tadpole_tflav_kernel
  {
    public:
    static inline auto kernel(const double& l1, const double& cos1, const double& p, const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory>& GK, const SplineInterpolator1D<double, LogarithmicCoordinates1D<double>, GPU_memory>& GKdot)
    {
      ...
      double fenv[(DiFfRG::kaon_tadpole_tflav::nenv) > 0 ? (DiFfRG::kaon_tadpole_tflav::nenv) : 1];
      DiFfRG::kaon_tadpole_tflav::fill(fenv, l1, cos1, p);
      const auto _interp1 = GK(sqrt(powr<2>(l1)));
      const auto _interp2 = GKdot(sqrt(powr<2>(l1)));
      return -0.5 * DiFfRG::kaon_tadpole_tflav::tr0(fenv) * powr<2>(_interp1) * _interp2;
    }
...
using DiFfRG::Kaon_tadpole_tflav_kernel;