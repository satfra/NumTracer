// GENERATED by gen_epsfund_num.cpp — do not edit.
#pragma once
#include <cmath>
namespace numtracer_kernels { namespace epsfund_num {
template<int N> static inline double powr(double x){ double r=1.0; for(int i=0;i<N;++i) r*=x; return r; }
// fundamental-symbol env layout (fill f[i] per call):
//   f[0] = var(0)
//   f[1] = var(1)
//   f[2] = var(4)
static inline constexpr int nenv = 3;
static inline void fill(double *f, [[maybe_unused]] double l1, [[maybe_unused]] double cos1, [[maybe_unused]] double cos2, [[maybe_unused]] double p) {
  f[0] = l1;
  f[1] = cos1;
  f[2] = p;
}
static inline double tr0([[maybe_unused]] const double *f) {
  const double s0 = f[2];
  const double s1 = f[1];
  const double s2 = s0*s1;
  const double s3 = f[0];
  const double s4 = s2*s3;
  const double s6 = s4*(2);
  return s6;
}
static inline double tr1([[maybe_unused]] const double *f) {
  const double s0 = f[2];
  const double s1 = f[1];
  const double s2 = s0*s1;
  const double s3 = f[0];
  const double s4 = s2*s3;
  const double s6 = s4*(4);
  return s6;
}
static inline double tr2(const double *f) { return tr1(f); }
static inline double tr3(const double *f) { return tr0(f); }
static inline double tr4([[maybe_unused]] const double *f) {
  const double s0 = f[2];
  const double s1 = f[1];
  const double s2 = s0*s1;
  const double s3 = f[0];
  const double s4 = s2*s3;
  const double s6 = s4*(3);
  return s6;
}
static inline double tr5([[maybe_unused]] const double *f) {
  const double s0 = f[2];
  const double s1 = f[1];
  const double s2 = s0*s1;
  const double s3 = f[0];
  const double s4 = s2*s3;
  const double s6 = s4*(9);
  return s6;
}
static inline double tr6(const double *f) { return tr5(f); }
static inline double tr7(const double *f) { return tr4(f); }
}} // namespace numtracer_kernels::epsfund_num
