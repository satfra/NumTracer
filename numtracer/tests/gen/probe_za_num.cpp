...
static inline auto probe_proj(const double& l1, const double& cos1, const double& p, const double& k)
{
  ...
  const auto _interp9 = ntRe(DiFfRG::za_num::tr0(fenv));
  const auto _interp10 = ntStub(62323., 0.816496580927726 * sqrt(powr<2>(l1) - cos1 * l1 * p + powr<2>(p)));
  const auto _interp11 = ntStub(29441., 1. * sqrt(powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
  ...
  const auto _interp14 = ntStub(27736., 0.816496580927726 * sqrt(powr<2>(l1) - cos1 * l1 * p + powr<2>(p)));
  const auto _interp15 = ntStub(20621., 1. * l1);
  const auto _interp16 = ntStub(20621., 1. * k);
  const auto _interp17 = ntStub(20621., 1. * sqrt(powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
  const auto _interp18 = ntStub(20621., 1.02 * k);
  const auto _interp19 = ntStub(95661., 1. * k);
  const auto _interp20 = ntRe(DiFfRG::za_num::tr3(fenv));
  const auto _den1 = powr<-1>(1. + powr<6>(k));
  const auto _den2 = powr<-2>(_interp4 * _interp5 + _interp3 * powr<2>(l1));
  const auto _den3 = powr<-2>(_interp16 * _interp5 + _interp15 * powr<2>(l1));
  const auto _den4 = powr<-1>(_interp12 * _interp4 + _interp11 * (powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
  const auto _den5 = -powr<-1>(_interp12 * _interp16 + _interp17 * (powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
  ...
  const auto _cse9 = -_interp16;
  const auto _cse10 = _cse9 + _interp18;
  const auto _cse11 = 50. * _cse10;
  const auto _cse12 = _cse11 + _interp19;
  const auto _cse13 = _cse12 * _interp5;
  const auto _cse14 = _interp16 * _interp6;
  const auto _cse15 = _cse13 + _cse14;
  return 0.02083333333333333 * fma(-2., _cse1 * _cse15 * _den3 * _den5 * _interp13 * powr<2>(_interp14), fma(-1., _cse1 * _cse8 * _den2 * _interp1 * _interp2, fma(2., _cse1 * _cse15 * _den3 * _den5 * powr<2>(_interp14) * _interp20, fma(-2., _cse1 * _cse8 * _den2 * _den4 * powr<2>(_interp10) * _interp9, 0.))));
}
static inline auto probe_repart(const double& l1, const double& cos1, const double& p, const double& k)
{
  ...
  const auto _interp10 = ntStub(30415., 1. * sqrt(powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
  const auto _interp11 = ntStub(80376., 1. * powr<2>(k) + 1.618033988749895 * (powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
  const auto _interp12 = ntStub(85453., 1. * sqrt(powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
  const auto _interp13 = ntIm(DiFfRG::za_num::tr5(fenv));
  const auto _interp14 = ntRe(DiFfRG::za_num::tr1(fenv));
  const auto _interp15 = ntStub(13076., 0.7071067811865475 * sqrt(powr<2>(l1) + powr<2>(p)));
  ...
  const auto _interp22 = ntRe(DiFfRG::za_num::tr0(fenv));
  const auto _interp23 = ntStub(62323., 0.816496580927726 * sqrt(powr<2>(l1) - cos1 * l1 * p + powr<2>(p)));
  const auto _interp24 = ntStub(29441., 1. * sqrt(powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
  const auto _interp25 = ntStub(89189., 1. * powr<2>(k) + 1.618033988749895 * (powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
  const auto _interp26 = ntRe(DiFfRG::za_num::tr2(fenv));
  const auto _interp27 = ntStub(27736., 0.816496580927726 * sqrt(powr<2>(l1) - cos1 * l1 * p + powr<2>(p)));
  const auto _interp28 = ntStub(20621., 1. * l1);
  const auto _interp29 = ntStub(20621., 1. * k);
  const auto _interp30 = ntStub(20621., 1. * sqrt(powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
  const auto _interp31 = ntStub(20621., 1.02 * k);
  const auto _interp32 = ntStub(95661., 1. * k);
  const auto _interp33 = ntRe(DiFfRG::za_num::tr3(fenv));
  const auto _den1 = powr<-1>(1. + powr<6>(k));
  const auto _den2 = powr<-2>(_interp17 * _interp18 + _interp16 * powr<2>(l1));
  const auto _den3 = powr<-2>(_interp18 * _interp29 + _interp28 * powr<2>(l1));
  const auto _den4 = powr<-1>(_interp17 * _interp25 + _interp24 * (powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
  const auto _den5 = -powr<-1>(_interp25 * _interp29 + _interp30 * (powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p)));
  const auto _den6 = powr<-2>(powr<2>(_interp8) + powr<2>(_interp4 * _interp5 + _interp9 * l1));
  const auto _den7 = powr<-1>(powr<2>(_interp10) + powr<2>(_interp11 * _interp4 + _interp12 * sqrt(powr<2>(l1) - 2. * cos1 * l1 * p + powr<2>(p))));
  const auto _cse1 = powr<-1>(l1);
  const auto _cse2 = powr<-2>(p);
  const auto _cse3 = -_cse1 * _interp3 * _interp4;
  const auto _cse4 = -_interp4;
  const auto _cse5 = _cse4 + _interp7;
  const auto _cse6 = 50. * _cse5;
  const auto _cse7 = _cse6 + _interp6;
  const auto _cse8 = -_cse1 * _cse7 * _interp5;
  const auto _cse9 = _cse3 + _cse8;
  const auto _cse10 = _interp17 * _interp19;
  const auto _cse11 = -_interp17;
  const auto _cse12 = _cse11 + _interp21;
  const auto _cse13 = 50. * _cse12 * _den1 * powr<6>(k);
  const auto _cse14 = _cse13 + _interp20;
  const auto _cse15 = _cse14 * _interp18;
  const auto _cse16 = _cse10 + _cse15;
  const auto _cse17 = _interp19 * _interp29;
  const auto _cse18 = -_interp29;
  const auto _cse19 = _cse18 + _interp31;
  const auto _cse20 = 50. * _cse19;
  const auto _cse21 = _cse20 + _interp32;
  const auto _cse22 = _cse21 * _interp18;
  const auto _cse23 = _cse17 + _cse22;
  return 0.02083333333333333 * fma(-1., _cse16 * _cse2 * _den2 * _interp14 * _interp15, fma(-4., _cse2 * _cse9 * _den6 * _den7 * _interp1 * powr<2>(_interp2), fma(4., _cse2 * _cse9 * _den6 * _den7 * _interp13 * powr<2>(_interp2), fma(-2., _cse16 * _cse2 * _den2 * _den4 * _interp22 * powr<2>(_interp23), fma(-2., _cse2 * _cse23 * _den3 * _den5 * _interp26 * powr<2>(_interp27), fma(2., _cse2 * _cse23 * _den3 * _den5 * powr<2>(_interp27) * _interp33, 0.))))));
}
