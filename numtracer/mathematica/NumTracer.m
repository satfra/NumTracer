...
NTFlowSelectedQ::usage = "NTFlowSelectedQ[name] is True when the flow `name` is part of the current selection. The selection comes from the NT_FLOWS environment variable (a comma-separated list of flow names); unset or empty selects EVERY flow, so an ungated script is unaffected. Read fresh on every call, so SetEnvironment[\"NT_FLOWS\" -> ...] works from a .wls that has already loaded the package.";

NTFlowGate::usage = "NTFlowGate[name, body] evaluates `body` only when NTFlowSelectedQ[name]; otherwise it prints one skip line and returns Null. HoldRest, so a skipped flow costs nothing -- wrap the WHOLE flow (the FunKit derivative and NumTrace included, not just the emit call), or the expensive part still runs. This is what lets one derivation file be sliced across several processes: run it N times with disjoint NT_FLOWS, then aggregate once with UpdateNTFlows (which rebuilds flows/CMakeLists.txt by scanning the flows directory, so it does not care which process wrote what).";

NTFlowSliceQ::usage = "NTFlowSliceQ[] is True when an NT_FLOWS selection is in force, i.e. this run emits a SLICE of the derivation file rather than all of it. A derivation file guards its final UpdateNTFlows with it: UpdateFlows builds flows/CMakeLists.txt from what is on disk, and in a slice the sibling processes have not finished writing their flows yet, so the aggregation must happen once, afterwards.";

NTFlowSelectionReport::usage = "NTFlowSelectionReport[] prints the NT_FLOWS entries that no NTFlowGate in this run ever matched -- i.e. the typos. Returns that list ({} when the selection was clean or absent). Call it at the END of a derivation file: a sliced run that silently emits nothing is otherwise indistinguishable from one that had nothing to do.";
